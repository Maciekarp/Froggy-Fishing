...
Varyings SplatmapVert(Attributes v)
{
    ...
    o.dynamicLightmapUV = v.texcoord * unity_DynamicLightmapST.xy + unity_DynamicLightmapST.zw;
        ...
        //float4 derivativeY = ddy(v.positionOS);
        //float4 derivativeX = ddx(v.positionOS); 
        //float3 crossTot = cross(derivativeY.xyz, derivativeX.xyz);
        //float3 normalized = normalize(crossTot.xyz);
        //o.normal = cross(float3(1, 0, 0), float3(0, 1, 0));
        //o.normal = cross(, );
        //float3 temp = TransformObjectToWorldNormal(v.normalOS)
        //o.normal = normalize(cross(ddy(Attributes.positionWS), ddx()));
...
#endif
{
    ...
    //inputData.normalWS = normalize(cross(ddy(Attributes.positionWS), ddx(Attributes.positionWS)));
    smoothness *= alpha;
